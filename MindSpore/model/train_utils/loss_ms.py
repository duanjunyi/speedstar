...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    ...
    box2 = box2.T
    op_min, op_max, op_pow, op_atan = ops.Minimum(), ops.Maximum(), ops.Pow(), ops.Atan()
    ...
    inter = clamp(op_min(b1_x2, b2_x2) - op_max(b1_x1, b2_x1), 0) * \
            clamp(op_min(b1_y2, b2_y2) - op_max(b1_y1, b2_y1), 0)
    ...
    if GIoU or DIoU or CIoU:
        cw = op_max(b1_x2, b2_x2) -op_min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = op_max(b1_y2, b2_y2) - op_min(b1_y1, b2_y1)  # convex height
...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        ####
        ai = self.repeat(Tensor(np.arange(na), dtype.float32).view(na, 1), (1, nt))  # same as .repeat_interleave(nt)
